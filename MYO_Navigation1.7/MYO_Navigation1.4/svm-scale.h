...
char *line_scale = NULL;
int max_line_len_scale = 1024;
...
int main_scale(int argc,char **argv)
{
	...
	line_scale = (char *)malloc(max_line_len_scale*sizeof(char));
...
char* readline_scale(FILE *input)
{
	...
	if (fgets(line_scale, max_line_len_scale, input) == NULL)
		return NULL;

	while (strrchr(line_scale, '\n') == NULL)
	{
		max_line_len_scale *= 2;
		line_scale = (char *)realloc(line_scale, max_line_len_scale);
		len = (int)strlen(line_scale);
		if (fgets(line_scale + len, max_line_len_scale - len, input) == NULL)
			break;
	}
	return line_scale;
}
